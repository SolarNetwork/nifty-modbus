...
public class SerialChannel extends AbstractNioByteChannel implements DuplexChannel {
	...
	public SerialChannel(SerialPortProvider serialPortProvider) {
		this(serialPortProvider, null, new SerialSocketChannel(new SerialSelectorProvider()));
	 ...
	 *        the serial port provider
	 * @param parent
	 *        the parent {@link Channel} by which this instance was created. May
	 *        be {@code null}
	 * @param ch
	 *        the underlying {@link SelectableChannel} on which it operates
	 */
	protected SerialChannel(SerialPortProvider serialPortProvider, Channel parent,
	...
	protected int doReadBytes(ByteBuf buf) throws Exception {
		...
		logger.debug("Read {} bytes from the wire", count);
		return count;
	}
	...
	protected int doWriteBytes(ByteBuf buf) throws Exception {
		...
		buf.readBytes(bytes);
		int count = serialPort.write(bytes, 0, expectedToWrite);
		logger.debug("Wrote {} bytes to wire", count);
		return count;
	}
	...
	protected boolean doConnect(SocketAddress remoteAddress, SocketAddress localAddress)
		...
		if ( !(remoteAddress instanceof SerialAddress) ) {
			throw new IllegalArgumentException(
					"Socket Address has to be of type " + SerialAddress.class);
		}
		logger.debug("Connecting to Socket Address '{}'", ((SerialAddress) remoteAddress).name());

		try {
			serialPort = serialPortProvider.getSerialPort(((SerialAddress) remoteAddress).name());
			logger.debug("Using Com Port {}, trying to open port", serialPort.getName());
			serialPort.open(config);
			if ( serialPort.isOpen() ) {
				logger.debug("Opened port successful to {}", serialPort.getName());
				serialPort.registerSelectionKey(serialSelectionKey);

				this.active = true;
				return true;
			} else {
				logger.debug("Unable to open port {}", serialPort.getName());
				return false;
			}
		} catch ( Exception e ) {
	...
	private class SerialNioUnsafe implements NioUnsafe {
		...
		public SerialNioUnsafe() {
			try {
				Constructor<ChannelOutboundBuffer> ctor = ChannelOutboundBuffer.class
						.getDeclaredConstructor(AbstractChannel.class);
		...
		public void register(EventLoop eventLoop, ChannelPromise promise) {
			...
			if ( !(promise.channel() instanceof SerialChannel) ) {
				throw new IllegalArgumentException("Only valid for " + SerialChannel.class + " but is "
						+ promise.channel().getClass());
			}
			...
			try {
				...
				serialSelectionKey = (SerialSelectionKey) ((SerialChannel) promise.channel())
						.javaChannel().register(selector, 0, SerialChannel.this);

				// Set selection key
				final Field selectionKeyField = AbstractNioChannel.class
						.getDeclaredField("selectionKey");
				...
				if ( !(pipeline() instanceof DefaultChannelPipeline) ) {
					throw new IllegalStateException(
							"Pipeline should be of Type " + DefaultChannelPipeline.class);
				}
				// Again reflection, but has to be done in an event loop
				eventLoop().execute(() -> {
					try {
						final Method invokeHandlerAddedIfNeeded = DefaultChannelPipeline.class
								.getDeclaredMethod("invokeHandlerAddedIfNeeded");
				...
				promise.setSuccess();
					...
					| ClosedChannelException | NoSuchFieldException e ) {
				logger.warn("Exception caught", e);
				throw new IllegalStateException("Should register channel to event loop!!!", e);
			}
		}
			...
			eventLoop().execute(() -> {
				try {
					...
					if ( success ) {
						...
						promise.setFailure(new RuntimeException("Unable to open the serial port '"
								+ ((SerialAddress) remoteAddress).name() + "'"));
					}
				} catch ( Exception e ) {
		...
		public void close(ChannelPromise promise) {
			logger.debug("Closing serial port [{}]",
					((SerialAddress) SerialChannel.this.remoteAddress).name());
			eventLoop().execute(() -> {
				try {
					doClose();
					promise.setSuccess();
		...
		public void write(Object msg, ChannelPromise promise) {
			...
			try {
				...
				Method estimatorHandle = DefaultChannelPipeline.class
						.getDeclaredMethod("estimatorHandle");
				estimatorHandle.setAccessible(true);
				MessageSizeEstimator.Handle handle = (MessageSizeEstimator.Handle) estimatorHandle
						.invoke(pipeline);
				// end of reflection
		...
		private void callFailFlushed(boolean notify) {
			try {
				Method failFlushed = ChannelOutboundBuffer.class.getDeclaredMethod("failFlushed",
						Throwable.class, boolean.class);
